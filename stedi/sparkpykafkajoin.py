...
zSetEntriesEncodedStreamingDF = zSetEntriesEncodedStreamingDF \
...
zSetEntriesEncodedStreamingDF.withColumn("customer", from_json("customer", customerJsonSchema)) \
    .select(col("customer.*")) \
...
emailAndBirthDayStreamingDF = emailAndBirthDayStreamingDF.select("email", "birthYear")
...
scoreStreamingDF = customerRiskStreamingDF.join(emailAndBirthDayStreamingDF, expr("customer = email"))
